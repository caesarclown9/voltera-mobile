...
## [1.1.4] - Build 101 - 2025-12-13 🔋 **Demo Charging Mode**

### 🎯 Цель: Полноценный демо-режим зарядки для Apple Review

Добавлена симуляция процесса зарядки для демо-станции VOL-001-01, чтобы
Apple рецензенты могли полностью протестировать функционал зарядки без
реального OCPP-подключения.

### ✅ Добавлено

#### 1. 🔋 Демо-режим зарядки в evpowerApi.ts

- **Проблема:** Демо-пользователь мог войти, но не мог запустить зарядку
- **Решение:** Добавлена полная симуляция процесса зарядки
- **Файл:** `src/services/evpowerApi.ts`
- **Компоненты:**
  - `DEMO_STATION_ID`, `DEMO_USER_ID` - константы демо-режима
  - `DemoSessionState` - интерфейс состояния демо-сессии
  - `getDemoChargingProgress()` - симуляция прогресса зарядки (7.5 kW)
  - `startCharging()` - демо-запуск зарядки с сохранением состояния
  - `getChargingStatus()` - симуляция статуса с расчетом энергии/стоимости
  - `stopCharging()` - демо-остановка с финальными данными
  - `getBalance()` - возврат 1000 KGS для демо-пользователя

#### 2. 📍 Исправлена валидация демо-номера

- **Проблема:** Номер +996123456789 не проходил валидацию оператора
- **Решение:** Добавлен bypass для демо-номера в PhoneInputForm.tsx
- **Файл:** `src/features/auth/components/PhoneInputForm.tsx:81-104`

#### 3. 🗄️ Станция VOL-001-01 активирована в БД

- **Изменения в Supabase:**
  - `stations.is_available = true` для VOL-001-01
  - `stations.last_heartbeat_at = NOW()` для обновления heartbeat
  - `connectors.status = 'available'` для всех коннекторов станции

#### 4. 🗺️ Demo Override для карты и статуса станции

- **Проблема:** Станция показывала серый маркер на карте из-за проверки heartbeat
- **Решение:** Добавлен клиентский override статуса для демо-локации
- **Файл:** `src/services/evpowerApi.ts`
- **Компоненты:**
  - `DEMO_LOCATION_ID = "VOL-001"` - константа демо-локации
  - `applyDemoLocationOverride()` - функция override статуса локации
  - `getLocations()` - применяет override для всех путей возврата данных
  - `getStationStatus()` - возвращает hardcoded данные для демо-станции
- **Результат:** Демо-станция всегда отображается зелёной (available) на карте

### 📊 Симуляция зарядки

| Параметр | Значение                  |
| -------- | ------------------------- |
| Мощность | 7.5 kW (0.125 kWh/min)    |
| Тариф    | 13.5 KGS/kWh              |
| Баланс   | 1000 KGS (фиксированный)  |
| Лимиты   | energy_kwh или amount_som |

### 🧪 Тестирование

1. Войти: `+996123456789` / `123456`
2. Баланс покажет 1000 сом
3. Станция VOL-001-01 (Ахунбаева) зелёная на карте
4. Запустить зарядку → симуляция процесса
5. Прогресс обновляется каждые 3 секунды
6. Остановить → финальные данные

### 📂 Измененные файлы

- `src/services/evpowerApi.ts` - демо-логика зарядки (lines 31-98, 424-458, 483-512, 608-624, 1018-1022)
- `src/features/auth/components/PhoneInputForm.tsx` - bypass валидации (lines 81-104)
- `src/lib/versionManager.ts` - версия 1.1.4 / build 101
- `package.json` - версия 1.1.4
