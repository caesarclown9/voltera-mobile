...
Проведён полный аудит производительности приложения. Исправлены проблемы с долгой загрузкой страниц и отсутствием обратной связи при запуске зарядки.

### ✅ Улучшено

#### 1. ⚡ Loading Overlay при запуске зарядки

- **Проблема:** При нажатии "Начать зарядку" 15-30 секунд ничего не происходило - пользователь не понимал, что процесс запущен
- **Корневая причина:** `chargingLoading = false` было захардкожено и никогда не менялось
- **Решение:**
  - Используем `isStarting` из хука `useCharging`
  - Добавлено локальное состояние `isStartingCharging`
  - Добавлен красивый полноэкранный оверлей с анимированным спиннером
  - Текст: "Запуск зарядки" + "Подождите, подключаемся к станции..."
- **Файлы:**
  - `src/pages/ChargingPage.tsx:47-52,81-83,239-266`
  - `src/i18n/locales/ru.json` - добавлены переводы
  - `src/i18n/locales/en.json` - добавлены переводы
  - `src/i18n/locales/ky.json` - добавлены переводы
- **Эффект:** Пользователь сразу видит, что процесс запущен

#### 2. 📊 Ленивая загрузка вкладок в HistoryPage

- **Проблема:** Загружались ВСЕ три API (charging, transactions, statistics) одновременно
- **Решение:** Загружаем данные только для активной вкладки
- **Файлы:**
  - `src/pages/HistoryPage.tsx:25-33`
  - `src/features/history/hooks/useChargingHistory.ts:55-62,136,143-150,206,213-220,367`
- **Эффект:** На ~66% меньше запросов при первой загрузке

#### 3. 🗄️ Кэширование статуса станции

- **Проблема:** `useStationStatus` с `staleTime: 0, gcTime: 0` - каждый раз новый запрос
- **Решение:** `staleTime: 10s, gcTime: 60s` - данные кэшируются
- **Файл:** `src/features/locations/hooks/useLocations.ts:140-156`
- **Эффект:** Переход на страницу станции мгновенный если данные свежие

#### 4. 🏪 Оптимизация StationsList

- **Проблема:** DynamicPricingDisplay делал отдельный API запрос для каждой станции в списке
- **Решение:** Убран DynamicPricingDisplay - данные уже есть в `station.price_per_kwh`
- **Файл:** `src/pages/StationsList.tsx:5-8,237-238`
- **Эффект:** N меньше запросов (где N = количество станций)

#### 5. 🚀 DataPrefetcher - предзагрузка данных

- **Добавлено:** Новый компонент `DataPrefetcher` загружает локации и станции в фоне
- **Когда:** Через 500мс после старта приложения
- **Файлы:**
  - `src/shared/components/DataPrefetcher.tsx` (новый)
  - `src/App.tsx:15,130`
- **Эффект:** Мгновенное открытие карты и списка станций
...
- `src/features/history/hooks/useChargingHistory.ts` - параметр enabled для хуков
...
- ✅ Production build: успешно (38.11s)
