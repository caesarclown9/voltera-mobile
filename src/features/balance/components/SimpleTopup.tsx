  ...
  const startPaymentStatusPolling = (invoiceId: string) => {
    ...
    setTimeout(() => {
      ...
      try {
        ...
        if (status.status === 1 && status.success === true && status.paid_amount > 0) {
          // Payment successful - confirmed paid
          console.log('Payment confirmed! Paid amount:', status.paid_amount)
          stopPolling()
          setStep('success')
          // Refetch balance to update it
      ...
      setTimeout(() => {
        stopPolling()
        setError('Время ожидания истекло. Если вы оплатили, баланс обновится автоматически.')
      }, 5 * 60 * 1000)
  ...
  const handleGenerateQR = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        startPaymentStatusPolling(result.invoice_id)
      }
    } catch (error: any) {
            ...
            <div className="space-y-4">
              ...
              {numAmount > 0 && (
                <div className="bg-gray-50 rounded-lg p-3 text-center">
                  ...
                  <span className="text-lg font-bold text-primary-600">{numAmount} сом</span>
                </div>
              )}
            ...
            <div className="space-y-4">
              <div className="text-center">
                {qrData?.qr_code_url ? (
                  ...
                  // If qr_code is base64, check if it's already a URL or needs base64 prefix
                  <img 
                    src={qrData.qr_code.startsWith('http') ? qrData.qr_code : `data:image/png;base64,${qrData.qr_code}`} 
                    alt="QR код"
                    className="w-48 h-48 mx-auto border-2 border-gray-200 rounded-lg p-2"
                  />
                ) : (
                ...
                {checkingPayment && (
                  <div className="mt-3 flex items-center justify-center gap-2">
                    ...
                    <span className="text-sm text-gray-600">Ожидаем подтверждение платежа...</span>
                  </div>
                )}
            ...
            <div className="space-y-4">
              <div className="text-center py-8">
                ...
                <p className="text-lg text-gray-600">
                  На сумму: <span className="font-bold text-primary-600">{numAmount} сом</span>
                </p>
