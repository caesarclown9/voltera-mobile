...
export const useQRTopup = () => {
  return useMutation<TopupQRResponse, Error, { client_id: string; amount: number; description?: string }>({
...
export const useQRPaymentFlow = () => {
  ...
  const createQRTopup = async (amount: number, clientId: string, description?: string) => {
    try {
      ...
      return {
        success: false,
        data: null,
        error: error.message || 'Ошибка создания QR кода',
      }
    }
  }
  ...
  const cancelPayment = async (invoiceId: string) => {
    try {
      ...
      return {
        success: false,
        message: error.message || 'Ошибка отмены платежа',
      }
    }
  }
