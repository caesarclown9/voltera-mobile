...
export function ChargingStatus({ sessionId, onSessionEnd }: ChargingStatusProps) {
  ...
  const getStatusText = (status: ChargingSession['status']) => {
    switch (status) {
      case 'preparing': return 'Подготовка к зарядке'
      case 'charging': return 'Идет зарядка'
      case 'suspended': return 'Зарядка приостановлена'
      case 'finishing': return 'Завершение зарядки'
      case 'finished': return 'Зарядка завершена'
      case 'faulted': return 'Ошибка зарядки'
  ...
  const getStatusColor = (status: ChargingSession['status']) => {
    switch (status) {
      ...
      case 'finishing': return 'text-blue-600 bg-blue-50'
      case 'finished': return 'text-gray-600 bg-gray-50'
      case 'faulted': return 'text-red-600 bg-red-50'
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          <div className="text-2xl font-bold text-gray-900">
            {session.energy.toFixed(2)} <span className="text-lg font-normal">кВт⋅ч</span>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          <div className="text-2xl font-bold text-gray-900">
            {session.power.toFixed(1)} <span className="text-lg font-normal">кВт</span>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          <div className="text-2xl font-bold text-green-600">
            {session.cost.toFixed(2)} <span className="text-lg font-normal">сом</span>
          </div>
        </div>
      </div>
