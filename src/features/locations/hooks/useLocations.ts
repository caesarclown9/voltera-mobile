...
export function useLocations(requestGeolocation: boolean = false) {
  ...
  const { data: locations, isLoading, error, refetch } = useQuery({
    ...
    placeholderData: () => {
      // Use cached data as placeholder while fetching new data
      return queryClient.getQueryData(['locations']) as Location[] | undefined;
    }
  });
 ...
 */
export function useLocation(locationId: string, includeStations: boolean = true) {
  const queryClient = useQueryClient();
  ...
  return useQuery({
    ...
    placeholderData: () => {
      ...
      return queryClient.getQueryData(['location', locationId, includeStations]) as Location | undefined;
    }
  });
}
...
export function useLocationUpdates(channels: string[] = ['all']) {
  ...
  useEffect(() => {
    ...
    // Подписываемся на нужные каналы
    channels.forEach(channel => {
      websocketManager.subscribe(channel);
    });

    // Обработчик обновлений
    const handleUpdate = (data: WebSocketLocationUpdate | WebSocketStationUpdate) => {
      if (data.type === 'location_status_update') {
        // Обновляем кэш локаций
        queryClient.setQueryData(['locations'], (oldData: Location[] | undefined) => {
          if (!oldData) return oldData;
          
          return oldData.map(location => 
            location.id === data.location_id 
              ? { ...location, status: data.status, stations_summary: data.stations_summary }
              : location
          );
        });

        // Обновляем кэш конкретной локации
        queryClient.invalidateQueries({ queryKey: ['location', data.location_id] });
      }

      if (data.type === 'station_status_update') {
        // Обновляем кэш статуса станции
        queryClient.setQueryData(['station-status', data.station_id], (oldData: StationStatusResponse | undefined) => {
          if (!oldData) return oldData;
    ...
    websocketManager.addUpdateHandler(handleUpdate);
    ...
    return () => {
      ...
      websocketManager.removeUpdateHandler(handleUpdate);
    };
  }, [channels.join(','), queryClient]);

  return {
    isConnected: websocketManager.isConnected(),
    subscriptions: websocketManager.getSubscriptions()
  };
}
...
export function useStations(requestGeolocation: boolean = false) {
  console.warn('useStations from useLocations is deprecated. Use features/stations/hooks/useStations instead')
  ...
  const { data: stations, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      // Импортируем адаптер
      const { extractStationsFromLocations } = await import('../../stations/types');
