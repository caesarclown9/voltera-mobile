...
export function StationCard({ station, onSelect, showDistance = true }: StationCardProps) {
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-md transition-shadow">
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <div className="flex items-start justify-between">
            <h3 className="font-semibold text-gray-900 text-lg">{station.locationName || station.model}</h3>
            {isAuthenticated && (
              <button
                onClick={() => toggleFavorite(station.id)}
                disabled={isToggling}
                className="ml-2 p-1 rounded-full hover:bg-gray-100 transition-colors"
                title={isFavorite(station.id) ? 'Удалить из избранного' : 'Добавить в избранное'}
              >
                <Heart
          ...
          <p className="text-gray-600 text-sm mt-1">{station.locationAddress || 'Адрес не указан'}</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
        <div className="flex items-center space-x-2">
          ...
          <span className="font-medium">{station.power_capacity || 'N/A'} кВт</span>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="font-medium">{station.price_per_kwh || 'N/A'} сом/кВт⋅ч</span>
        </div>
        ...
        {showDistance && station.distance !== undefined && (
          <div className="flex items-center space-x-2">
            ...
            <span className="font-medium">{formatDistance(station.distance)}</span>
          </div>
        )}
      </div>
      ...
      <div className="flex space-x-2">
        <button
          onClick={onSelect}
          disabled={station.status === 'inactive' || station.status === 'maintenance'}
      ...
      {station.status !== 'active' && (
        <div className={`mt-3 p-2 rounded-lg ${
          ...
          <p className={`text-xs ${
            station.status === 'maintenance' ? 'text-purple-700' : 'text-gray-600'
          }`}>
            {station.status === 'maintenance' && 'Станция временно недоступна из-за технических работ'}
            {station.status === 'inactive' && 'Станция не отвечает. Обратитесь в службу поддержки'}
          </p>
        </div>
      )}
    </div>
