  ...
  try {
    ...
    if (Capacitor.isNativePlatform()) {
      ...
      try {
        ...
        if (initialized) {
          logger.debug('Platform: Push notifications initialized')

          // Пробуем зарегистрировать устройство для получения токена
          const hasPermission = await pushNotificationService.checkPermissions()
          if (hasPermission) {
            const token = await pushNotificationService.register()
            if (token) {
              logger.info('Platform: Device registered for push notifications')
              // TODO: Отправить токен на backend
            }
          }
        }
      } catch (error) {
        logger.warn('Platform: Push notifications initialization failed', error)
      }
