        ...
        <div className="flex items-center justify-between p-4 border-b">
          <div>
            ...
            <p className="text-gray-600 text-sm">
              {stations.length} станци{stations.length === 1 ? 'я' : stations.length < 5 ? 'и' : 'й'}
            </p>
          </div>
          <button 
            onClick={onClose}
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-2 mt-1">
                        {connectorStatus.map((connector) => (
                          <div key={connector.connector_id} className="flex items-center space-x-1">
                            <span className={`w-2 h-2 rounded-full ${
                              connector.status === 'Available' ? 'bg-green-500' :
                              connector.status === 'Charging' ? 'bg-orange-500' : 'bg-red-500'
                            }`} />
