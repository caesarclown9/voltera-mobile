...
export const validators = {
  ...
  pattern: (regex: RegExp, message = 'Неверный формат'): ValidationRule<string> => ({
  ...
  phone: (message = 'Неверный формат номера телефона'): ValidationRule<string> => ({
    validate: (value) => {
      if (!value) return true
  ...
  range: (min: number, max: number, message?: string): ValidationRule<number> => ({
    validate: (value) => value !== undefined && value !== null && value >= min && value <= max,
  ...
  amount: (min = 0, max = 100000, message?: string): ValidationRule<number> => ({
    validate: (value) => {
